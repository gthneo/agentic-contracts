...
| 人占闸与 `parallel` 及以上档 | **本契约的人机互斥只适用于 `parallel` 以下档。** 依据 0 号宪法不变式 4 的按档区分：`parallel`/`audit`/`auto` 下人发言是**同席信号、不是自降信号**（那几档的定义就是人和 Agent 都在发），此时不触发人占闸自降，靠总闸 / 手动降档 / 撤回收回。⚠️ 但**本契约的互斥是 GUI 层的**（Agent 与人抢同一个客户端界面），与配比档正交：**只要 Agent 的写路径仍需驱动 GUI，`parallel` 及以上在本渠道就不可实现**——须待后端提供非 GUI 的发送通路（或明确声明可并发驱动）后才谈得上。实现方声明前，本渠道封顶 `semi`。 |
