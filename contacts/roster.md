...
| **⚠️ 序列化约定不一致(待 AMR 钉)** | 后端 `Contact` 结构带 `#[serde(rename_all = "camelCase")]` → 实际出线是 **camelCase**(`nickName`/`smallHeadUrl`/`contactType`/`localType`/`isFriend`)。而 §2 JSON 示例**混用两种约定**(`nickName` camelCase 但 `id_scheme`/`entity_kind`/`small_head_url` snake_case)。**请 AMR 钉一种**:(a) 新字段随现网 camelCase(`idScheme`/`entityKind`/`friend`),PD 零改;或 (b) 契约坚持 snake_case,PD 加 serde alias 双写。**同一问题含 `friend` vs `is_friend` 命名**:§2.1 表定 `friend`,而 AMR 回执 handoff(07-21 §4)请求 `is_friend`;PD 当前按 handoff 出 `isFriend`。**钉死后 PD 即刻改齐**(改名/双写皆可)。 |
