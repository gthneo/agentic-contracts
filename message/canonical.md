...
> **⚠️ out 落地守宪法**：`friend_verify`（主动发好友申请）/ `call`（发起通话）/ `channels`（发/转视频号）都是**对外动作**，且属 0 号宪法封顶表里的**「首次建立关系 / 一对多广播」= 不可逆**一类 —— 将来实现出向时**必须继承 outbox→confirm 人审闸，且档位封顶 `semi`（人亲手放行）**，实现方不得建更高档的自动发送路径。
