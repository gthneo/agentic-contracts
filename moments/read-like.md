...
- `text`（必填）= 要删除的评论正文——后端在该帖下匹配 self 发的、正文 == `text` 的评论删之。
  - ✅ **寻址口径（AMR 定稿 2026-07-21）= by-text（采纳 PD 提案）**：用 `text` 精确匹配 self 评论——AMR outbox 天然记得自己评了什么、有 `text`，无需后端另暴露评论读接口 / comment-id。**否掉备选「删最近一条」**：最近一条会误删人在原生微信手评的评论；by-text 只删 AMR 自己说过的话 = 误触回收精准、账号资产保护。定稿约束：
    ...
    - **多条同文 self 评论** → 删**最近一条匹配**（LIFO——最可能是误触新增的那条）。
...
> **AMR 定稿裁定 2026-07-21（本 spec 内联落定，无未决口径）**：①delete-comment = **by-text**（§4.7，仅 self 自评、同文删最近一条、无匹配 `COMMENT_NOT_FOUND` 不猜）；②sync 返回 = **§2 完整信封数组**（§3.4）；③warm 编排 + 撤销 HITL 边界归 AMR，撤销只动自己 outbox 动作（§5.5）；④错误码枚举**封闭集**归一（§4.8）；⑤§8.1 capabilities 漂移**按契约收敛**（`read`/对象 `publish`/`sync`），AMR 侧双读兼容层防 flag-day。删自己动态 backup 形态见 `publish.md` §7.5。*
