...
- 成功返回 warm 后可寻址的 moments：`{ "ok": true, "count": N, "moments": [ <§2 Moment Envelope>, ... ] }`。**每个 element = §2 完整信封**（与 `GET /api/moments?person=` 同形态），AMR 拿到即可直接渲染 + 选候选 `tid`，无需再跟一次 GET（省往返）。
...
- `text`（必填）= 要删除的评论正文——后端在该帖下匹配 self 发的、正文 == `text` 的评论删之。
  - ✅ **寻址口径（AMR 定稿 2026-07-21）= by-text（采纳 PD 提案）**：用 `text` 精确匹配 self 评论——AMR outbox 天然记得自己评了什么、有 `text`，无需后端另暴露评论读接口 / comment-id。**否掉备选「删最近一条」**：最近一条会误删人在原生微信手评的评论；by-text 只删 AMR 自己说过的话 = 误触回收精准、账号资产保护。定稿约束：
    - **仅匹配 self 自评**（微信只允许删自己评论；传他人评论正文 → `COMMENT_NOT_FOUND`）。
    - **多条同文 self 评论（歧义）** → 返 `COMMENT_AMBIGUOUS`，**后端不猜、不兜底删任何一条**；AMR 降级人工手删（与「无匹配」同款保守）。**否掉原 LIFO「删最近一条」**——独立宪法审（2026-07-21）指出：若人在原生微信手评了与 AMR outbox 同文、且更晚的评论，LIFO 会误删**人的**那条（删评论不可逆），与本条「绝不误删人手评」自相矛盾、且违「不确定就交回人」的保守铁律。歧义交回人，不用启发式猜。
    - **无匹配** → `COMMENT_NOT_FOUND`，**后端不猜、不兜底删别的**；AMR 降级人工手删（保守，绝不误删）。
    - **AMR 侧约束**：AMR 只对**自己 outbox 里评过的 text** 发 delete-comment，绝不请求删人在原生微信手评的评论（撤销面 = 自己动作面，见 §5.5）。
    - 未来若需在同文多评论里精确定位某一条 → v.next `comment_id` 寻址扩展（YAGNI，现不建）。
...
### 4.8 错误码枚举（AMR 定稿 2026-07-21·封闭集）

read-like 的写 / 撤 / warm 动作结构化 `error`（**封闭枚举**，新增走 AMR 改 spec；命名对齐 `send-target` 的 `CONTACT_NOT_FOUND`/`SEND_FAILED` 风格 = 名词前置 `<NOUN>_<STATE>` 或动词 `<VERB>_FAILED`）：

| `error` | 场景 | 端点 |
|---|---|---|
| `MOMENT_NOT_FOUND` | `tid` 不在（cold 未 warm / 已删） | like / unlike / comment / comment_delete |
| `PERSON_NOT_FOUND` | sync 的 person wxid 不存在 | sync |
| `SYNC_FAILED` | warm 导航执行失败（GUI/selector 异常） | sync |
| `UNLIKE_FAILED` | 取消赞执行失败 | unlike |
| `COMMENT_FAILED` | 评论执行失败 | comment |
| `COMMENT_DELETE_FAILED` | 删评论执行失败 | comment_delete |
| `COMMENT_NOT_FOUND` | 该帖下无匹配 self 评论（by-text 未命中） | comment_delete |
| `COMMENT_AMBIGUOUS` | 同文 self 评论 >1 条（歧义，后端不猜，降级人工） | comment_delete |

> 删自己动态（delete-moment）走姊妹 `publish.md`，其错误码见 `publish.md` §3.3 / §7.5（`MOMENT_NOT_OWNED` / `MOMENT_DELETE_FAILED` / `MOMENT_NOT_FOUND`）。**幂等 no-op（已达期望态：已赞再赞 / 已撤再撤 / 已删再删）一律返 `ok:true`，不进错误枚举。**
...
### 5.5 撤销面 = 自己动作面 + warm 编排（AMR 消费语义·可逆的 HITL 边界）

可逆（§1.7）落到编排的三条约束（AMR 侧决定，后端不实现）：

- **撤销走与正向同一道 HITL 闸**：unlike / delete-comment / delete-moment 都是**对外动作**，与 like/comment/publish 同级——**永不自动撤**，人审后逐条撤（哑执行端点）。撤销不是「系统自动纠错」，是「人 HITL 回收误触」。
- **AMR 只撤自己 outbox 里的动作**：候选撤销清单只列 **AMR 自己做过的** like/comment/publish（有 outbox 留痕的），**绝不把人在原生微信手动做的 like/comment 列入撤销候选**。守「撤销 = 回收自己的误触对外痕迹」语义，不越界动人的手动表态（呼应 §4.7 by-text 只删自评）。
- **warm 编排（§3.4）归 AMR**：sync-on-demand（对 cold 人首次动作前才 warm）+ per-person warm-state 缓存（warm 后 `tid` 长寿命 → 不重复 sync，省 GUI 秒级代价）+ 保守重 sync（仅在动作返 `MOMENT_NOT_FOUND` 时补 warm 再重试一次）。后端只提供哑执行 warm 端点。
...
| `liked` 精度 | 依赖 `sns.db` 本地赞记录同步状态；若后端确认无法可靠读取，声明 `"liked_reliable": false`（扩展字段），AMR 跳过本地幂等、仅依赖端点幂等。**权威判定 = 点赞/取消赞端点响应里的 `liked`**（后端内部经视觉 likers 行确认，§8）；envelope 里的 `liked` 仅作本地幂等提示（best-effort），person-scoped 帖已知结构性假阴。AMR 的 correctness **不赖 envelope.liked，赖端点 `ok`+`liked`**。 |
...
| 项 | 实现参考 |
|---|---|
| 动态读取 | 读 `sns.db SnsTimeLine`；`GET /api/moments?person=<wxid>` 后端已上线，`person` 过滤 + `liked` 字段 + 信封格式对齐本契约。 |
| 点赞 / 评论执行 | **✅ person-scoped 端到端已实证（真机 2026-07-20，班迪2727 对 3 位真实联系人最新帖各点赞 + 评论,likers 行 ❤ + 评论均截图坐实）**。路径 = tid 反解作者 → 搜人 → profile 卡 → 点卡 Moments 缩略图开相册 → 导航到目标帖 → 开单帖 **Details**（其 a11y 可靠）→ ⋯ → Like（⋯−217）/ Comment（⋯−84 + `Tab Tab space` 键盘发送）。`like`/`comment` 声明 `true`。原「a11y spike 未完成→like:false」注**已过时,推翻**。 |
| ⚠️ 定位可靠性（PD→AMR 事实） | **相册 WebView（XWeb/RadiumWMPF·Chromium 126）的 a11y 平台导出 + CDP 均被 Tencent 编译期封死**（真机 2026-07-20 逐步取证：注入 flag+总线地址均不生效）→ 相册帖列表**无结构化 introspection**，「相册里定位目标帖」这一 hop 当前靠视觉/几何（Details 一旦打开则 a11y 可靠）。**不影响契约接口形态**（tid 寻址 + 端点不变），仅影响后端定位鲁棒性 = **工程化收口进行中**（把手动验通链路固化进 person-scoped plan 的感知层）。研究全文见后端仓 `docs/design/webview-a11y-structural-research-2026-07-20.md`。 |
| person-scoped 寻址（§3.4） | **✅ 已实证**：`POST /api/moments/sync?person=<wxid>` 导航相册把 X 的帖 warm 进 `sns.db` 拿稳定 tid（真机 2026-07-20 张先生/仁兄/shirley count 11/28/22）。**端点已存在但后端 capabilities 暂未报 `sync`（见下 capabilities 漂移）。** |
| 媒体 `ref` | 复用已上线 `/api/media` 端点；朋友圈图片/视频的解密 + 取件逻辑与消息通道同族，统一 bearer auth。 |
| `tid` 稳定性 | `sns.db SnsTimeLine` tid = 服务器帖 id，缓存后稳定（导航过相册即持久，实测跨会话不变）；不用 `create_time`（时间戳不唯一）。 |
| `liked` 查询 | 从 `sns.db` 查 self 是否已赞。**⚠️ person-scoped 帖点赞不更新 `sns.db.liked`（结构性假阴）→ 权威确认退视觉 likers 行**（Details 下方 ❤ + self 头像）；`liked` 现值对全局 feed 帖有效。 |

### §8.1 capabilities 漂移 —— ✅ AMR 已定口径（2026-07-21 结项）

后端 `/api/capabilities.moments` 2026-07-20 **报告** = `{ "list": true, "publish": "text-only", "like": true, "comment": true }`，与本契约 §6.1 目标形态**有 3 处漂移**（PD 给事实，形态归属 AMR 定）：

| 漂移 | 后端现值 | 契约 §6.1 目标 | AMR 裁定（2026-07-21） |
|---|---|---|---|
| 读能力字段名 | `list` | `read` | **`read` 胜**（读语义动词，与 message read-contract 同族）;PD 改后端字段名 |
| publish 形态 | 字符串 `"text-only"` | 对象 `{text,image,link,video,delete}` | **改对象**（表达力+逐能力 gate）;PD 改后端映射 |
| person-scoped sync | **缺**（端点已存在） | `sync:true`(§3.4) | PD 补报 `sync:true`（additive·安全无 breakage） |

**D 角未实现项**（`unlike`/`comment_delete`/`publish.delete`）后端暂缺/报 `false`,与 §6.1「未实现前 false」一致,无漂移。**改 `list→read` / `publish` 字符串→对象为 breaking（现有消费者可能读旧形态）→ 等 AMR 定口径后 PD 一次改齐**;`sync:true` additive 可先补。

**✅ AMR 定稿裁定（2026-07-21）—— §6.1 目标形态即真相源，三处漂移全按契约收敛**：

1. **`list` → `read`**：契约统一用 `read`（读语义动词，与 `message` read-contract 同族）。后端改字段名。
2. **`publish` 字符串 `"text-only"` → 对象** `{text,image,link,video,delete}`：对象更有表达力、可逐能力 gate（§6.1）。后端改映射。
3. **`sync:true`**：additive，PD 已先补，收下。

**cutover 防 flag-day（AMR 侧工程纪律，写进本裁定以免部署先后踩空）**：AMR capability 解析器**过渡期同时容忍两形态**——读能力取 `caps.read ?? caps.list`；`publish` 为字符串时按 `{text:true, image/link/video/delete:false}` 解，为对象时直读。→ **两侧部署先后无所谓**，不设一刀切切换日。PD 后端改齐后，此双读兼容层**保留 ≥1 个契约版本再撤**（避免旧快照回滚踩空）。此裁定落地 = 本节从「待对齐」结项。
...
> **AMR 定稿裁定 2026-07-21（本 spec 内联落定，无未决口径）**：①delete-comment = **by-text**（§4.7，仅 self 自评；同文多条=歧义 `COMMENT_AMBIGUOUS` 降级人工、无匹配 `COMMENT_NOT_FOUND`，一律后端不猜）；②sync 返回 = **§2 完整信封数组**（§3.4）；③warm 编排 + 撤销 HITL 边界归 AMR，撤销只动自己 outbox 动作（§5.5）；④错误码枚举**封闭集**归一（§4.8）；⑤§8.1 capabilities 漂移**按契约收敛**（`read`/对象 `publish`/`sync`），AMR 侧双读兼容层防 flag-day。删自己动态 backup 形态见 `publish.md` §7.5。*

> **§8 实证更新 2026-07-20（PD 实现侧,不改接口形态）**：`like`/`comment` person-scoped **端到端真机实证**（3 位真实联系人最新帖各赞+评,截图坐实）→ 推翻原「a11y spike 未完成→like:false」；`sync`(§3.4) 实证 + 后端 additive 补报 `sync:true`；新增 **§8.1 capabilities 漂移事实**（后端现报 `list`/`publish:"text-only"` vs 契约 `read`/对象 → **已于 2026-07-21 定口径按契约收敛，见 §8.1 裁定 + AMR 双读兼容层**）；相册 WebView（XWeb·Chromium 126）introspection 被 Tencent 编译期封死之研究结论（不影响接口,仅定位鲁棒性=后端工程化收口）。
