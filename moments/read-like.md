...
### §8.1 capabilities 漂移 —— PD→AMR 待对齐事实（2026-07-20）

后端 `/api/capabilities.moments` **当前实际报告** = `{ "list": true, "publish": "text-only", "like": true, "comment": true }`，与本契约 §6.1 目标形态**有 3 处漂移**（PD 给事实，形态归属 AMR 定）：

| 漂移 | 后端现值 | 契约 §6.1 目标 | 对齐建议（待 AMR 拍口径） |
|---|---|---|---|
| 读能力字段名 | `list` | `read` | 二选一（`list`=后端惯用、`read`=契约新命名）;定后 PD 改后端一行 |
| publish 形态 | 字符串 `"text-only"` | 对象 `{text,image,link,video,delete}` | 改对象更表达力;PD 改后端映射 |
