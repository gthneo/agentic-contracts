...
- 自动沟通的**双闸**（话术库白名单 ∧ LLM 低风险）是**准入条件**，只覆盖**低风险会话回复**（寒暄/确认类安全话术）。
- **广播发布封顶 `semi`** —— 它是 0 号宪法封顶表里的**不可逆**动作（一对多、发出即扩散、撤回收不回已扩散的部分），
  **永不进 `parallel` / `audit` / `auto` 任何一档**，总是走 §7 的人审。
- 一句话：**会话内消息可随信任累积升到高档自动发；朋友圈广播永远要人亲手放行。**
