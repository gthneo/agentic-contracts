...
- **宪法敏感的功能 PR**（消费方仓里新增/改动一个**碰到对外动作、协作档位、人审窗、撤回窗、Agent 决策权**的功能）。
...
1. **有没有旁路人**——判据是「**人能不能收回**」，**不是**「人有没有经手每一条」（0 号宪法 v2 红线）：
   **总闸可断 / 档位可降 / 动作可撤 / 全程有痕**——四样缺一即违宪。逐项核：对外动作（send /
   publish / like）是否只在**人设定的协作档**内执行、每档都有**可介入手柄 + 撤回窗 + 留痕**？
   协作档是否仍**人手拨、系统不自升**，且**异常必自降到 `draft`**？不可逆动作（广播 / 删除 /
   建立关系）是否守住**封顶 `semi`**？结果是否**回交给人看**（粒度可随档变，但不得为零）？
   是否**另造了一套档位词汇**（宪法六档之外自立门户 = 隐蔽漂移）？
